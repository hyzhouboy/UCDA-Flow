...
class UCDAFlowDec(nn.Module):
    ...
    def forward(self, net, inp, corr, flow, itr, upsample=True):
        ...
        feat_moa = self.ucda(inp, feat_mo, itr)
...
class UCDA(nn.Module):
    ...
    def forward(self, *inputs):
        ...
        if itr == 0:
            ...
            attn_ucda = ker_S.view(attn.shape) * attn
            self.attn = F.softmax(attn_ucda, dim=1)
...
class UCDA_NonLocal_Match(nn.Module):
    def __init__(self, args, chnn, inter_channels=None, dimension=3, sub_sample=True, bn_layer=True):
        super().__init__()
